...
try {
    ...
    $receiptCategory = isset($_GET['receipt_category']) ? $conn->real_escape_string($_GET['receipt_category']) : 'Tuition Receipt';
    $studentId = isset($_GET['student_id']) ? trim($_GET['student_id']) : '';
    $offset = ($page - 1) * $limit;

    $where = "WHERE ct.receipt_category = '{$receiptCategory}'";

    if ($studentId !== '') {
        $studentIdEscaped = $conn->real_escape_string($studentId);
        $where .= " AND (u.student_id = '{$studentIdEscaped}' OR ct.guest_school_id = '{$studentIdEscaped}')";
    }
